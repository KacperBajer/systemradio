...
const PlaylistTable = ({data, checked, setChecked, fetchData}: Props) => {
  ...
  return (
    <>
        {showAddSongToPlaylistPopup && <AddSongToPlaylist handleClose={() => setShowAddSongToPlaylistPopup(false)} songs={[showAddSongToPlaylistPopup as number]} />}
        {showMoreBox && <MoreBox setChecked={setChecked} playlist={data} addToAnotherPlaylists={setShowAddSongToPlaylistPopup} id={showMoreBox} handleClose={() => setShowMoreBox(null)} position={moreBoxPosition as { top: number; left: number }} />}
        <div className='overflow-x-auto'>
            <div className='flex flex-col'>
                <div className='sticky top-0 flex items-center pb-2 border-b-2 border-dark-100 text-gray-300 text-sm px-4'>
                    <CustomCheckbox handleChange={handleSelectAllChange} isChecked={data.songs.length === checked.length} />
